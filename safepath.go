...
func (p *Path) Raw() *UnsafePath {
	return NewUnsafe(p.rootBase, p.relativePath)
}

func (p *Path) IsRoot() bool {
	return UnsafeAbsolute(p.Raw()) == pathRoot
}
...
func (p *Path) AppendAndResolveWithRelativeRoot(relativeRootElems ...string) (*Path, error) {
	tmpPath, err := JoinAndResolveWithRelativeRoot(UnsafeAbsolute(p.Raw()), relativeRootElems...)
	if err != nil {
		return nil, err
	}
...
func JoinNoFollow(rootPath *Path, path string) (*Path, error) {
	...
	p := newPath(UnsafeAbsolute(rootPath.Raw()), path)
	f, err := OpenAtNoFollow(p)
	if err != nil {
		return nil, err
	}
...
func ListenUnixNoFollow(socketDir *Path, socketName string) (net.Listener, error) {
	...
	addr, err := net.ResolveUnixAddr("unix", filepath.Join(UnsafeAbsolute(socketDir.Raw()), socketName))
	if err != nil {
		return nil, err
	}
